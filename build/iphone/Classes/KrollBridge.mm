...
@implementation MyTunesRSSObject
...
-(void)injectPatches
{
	// called to inject any MyTunesRSS patches in JS before a context is loaded... nice for 
...
-(void)didStartNewContext:(KrollContext*)kroll
{
	...
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mytunesrssNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mytunesrss valueForKey:name];
