...
{
	if (self = [super init]) {
		_mytunesrssRoot = [rootController retain];
...
-(void)dealloc
{
	RELEASE_TO_NIL(_mytunesrssRoot);
...
-(void)loadView
{
	...
	[[NSNotificationCenter defaultCenter] removeObserver:_mytunesrssRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	return [_mytunesrssRoot shouldAutorotateToInterfaceOrientation:interfaceOrientation];
}

-(void)refreshOrientationModesIfNeeded:(TiWindowProxy *)oldCurrentWindow
{
	[_mytunesrssRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
