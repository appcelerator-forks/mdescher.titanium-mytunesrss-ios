...
-(AudioStreamer*)player
{
	if (player==nil)
	{
		...
		if (progress)
		{
			...
			// [MDESCHER] add timer to mainRunLoop instead of currentRunLoop
			timer = [[NSTimer timerWithTimeInterval:1 target:self selector:@selector(updateProgress:) userInfo:nil repeats:YES] retain];
...
-(void)setAudioSessionMode:(NSNumber*)mode
{
	...
	NSLog(@"[WARN] 'MyTunesRSS.Media.AudioPlayer.audioSessionMode' is deprecated; use 'MyTunesRSS.Media.audioSessionMode'");
